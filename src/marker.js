L.SegmentMarker = L.FeatureGroup.extend({
  ...
  _getOpacity(order) {
    ...
    const opacity =
      stepOpacity + cx * this.options.opacityDecrease * stepOpacity;
    return opacity.toPrecision(6);
  },

  _makeCircle(distance, startAngle, endAngle, color) {
    const sequenceOrder =
      distance / (this.options.maxDist / this.options.noSteps);
  ...
  drawCircle(distance) {
    ...
    if (this.options.sequences.length === 1) {
      ...
      L.FeatureGroup.prototype.addLayer.call(
        this,
        this._makeCircle(distance, 0, 360, color).bringToFront()
      );
    }
    // more sequences
    else {
      for (var i = 0; i < 360 / this.options.circleSegmentAngle; i++) {
        ...
        L.FeatureGroup.prototype.addLayer.call(
          this,
          this._makeCircle(distance, sAngle, eAngle, color)
        );
      }
    }
  }
...
L.segmentMarker = options => new L.SegmentMarker(options);
